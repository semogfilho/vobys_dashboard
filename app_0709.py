...
import auth_ui, inicio, grafico, app_siafe, responsavel, arquivos, usuarios, auditoria_integracao, auditoria_folha, valida_bb
...
def get_opcoes():
    opcoes_base = ["Inicio", "Grafico", "Pagto Pendente", "Responsavel", "Arquivos/ID", "Auditoria", "Auditoria Folha"]
    # Perfil 'a' (Admin) ganha acesso a Usuários e Validação BB
    if st.session_state.get("perfil_usuario") == 'a':
        return opcoes_base + ["Usuários", "Validação BB"]
    elif st.session_state.get("perfil_usuario") in ['g', 'c']:
        return opcoes_base + ["Usuários"]
    return opcoes_base
...
def restaurar_sessao():
    ...
    if token_url:
        ...
        if dados and len(dados) == 3:
            ...
            # Se for um usuário novo, resetamos os dados sensíveis, 
            # mas não apagamos o menu_atual para evitar erros de renderização
            if st.session_state.get("login_atual") != login:
                st.session_state.autenticado = True
                st.session_state.nome_usuario = nome
                st.session_state.perfil_usuario = perfil
                st.session_state.login_atual = login
                # Se o usuário mudou, aí sim forçamos o menu para o início
                st.session_state.menu_atual = "Inicio" 
                return True
            return True
    return False
...
restaurar_sessao()
#st.sidebar.info(f"DEBUG - Perfil Recebido: {st.session_state.perfil_usuario}")
...
# CSS otimizado para compactar o topo
    ...
    # Se o dia for a partir de 23, mostra o mês seguinte
    if hoje.day >= 23:
        if hoje.month == 12:
...
# Demais módulos que não exigem SEFAZ
    ...
    elif menu_selecionado == "Auditoria Folha": auditoria_folha.render(conn, ano, mes)
    elif menu_selecionado == "Usuários": usuarios.render(conn, st.session_state.perfil_usuario)
    elif menu_selecionado == "Validação BB":
        # Verificação de segurança adicional para garantir que apenas Admin acesse
        if st.session_state.perfil_usuario == 'a':
            valida_bb.render(conn) # Certifique-se de que sua view tenha um método render(conn)
        else:

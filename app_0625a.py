...
# 2. LÓGICA DE CAPTURA DE IP
def capturar_ip_via_js():
    # Verifica se já capturamos antes de rodar o JS
    if "ip_usuario" not in st.session_state:
        if "ip_capturado" in st.query_params:
            st.session_state.ip_usuario = st.query_params["ip_capturado"]
        else:
            js_code = """
            <script>
...
# CSS otimizado para compactar o topo
    ...
    if hoje.day <= 12:
        if hoje.month == 1: mes_sugerido, ano_sugerido = 12, hoje.year - 1
        else: mes_sugerido, ano_sugerido = hoje.month - 1, hoje.year
    else: mes_sugerido, ano_sugerido = hoje.month, hoje.year
...
try:
    ...
    elif menu_selecionado == "Auditoria Folha": 
        sub_escolha = st.session_state.get("sub_auditoria", "Consistência Folha")
        auditoria_folha.render(conn, ano, mes, sub_escolha)

...
def carregar_dados_folha_oficial(conn, schema, chave, ano, mes):
    ...
    query = f"""
    WITH CPFs_Vigentes AS (
        SELECT id_pessoa, CPF_PESSOA
        FROM (
            SELECT id_pessoa, CPF_PESSOA,
                   ROW_NUMBER() OVER (
            ...
            -- 1. O cadastro deve ser anterior ou igual ao fim do mês da competência (para ignorar CPFs novos)
            -- 2. A baixa deve ser nula ou posterior ao fim do mês da competência (para garantir vigência)
            WHERE 1=1 --data_cadastro <= LAST_DAY(TO_DATE(:data_referencia, 'YYYY-MM-DD'))
              AND (data_baixa IS NULL OR data_baixa >= LAST_DAY(TO_DATE(:data_referencia, 'YYYY-MM-DD')))
        )
        WHERE rn = 1
    )
    SELECT
        doc.CPF_PESSOA,
        COALESCE(NULLIF(ff.matr_dependente, '0'), ff.cod_institucional) AS cod_institucional,
        p.NOME
    ...
    LEFT JOIN CPFs_Vigentes doc ON doc.id_pessoa = p.id_pessoa
    WHERE ff.ind_remuneracao = 'S' 

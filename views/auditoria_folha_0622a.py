...
def render(conn, ano_selecionado, mes_selecionado):
    ...
    # Correção do erro de digitação no placeholder do título: {{mes_nom}} -> {mes_nome}
    ...
    for schema in schemas:
        ...
        query_parts.append(f"""
            SELECT
                ...
                f.DATA_FECHAMENTO,
                fs.CODIGO_SEFAZ,
                (SELECT COUNT(*) FROM {schema}.FOLHA_FUNC ff WHERE ff.ID_FOLHA = f.ID_FOLHA) as QTDE_REGISTROS
            FROM {schema}.FOLHA f
    ...
    with st.spinner("Analisando inconsistências e carregando códigos SEFAZ..."):
        try:
            ...
            if not df.empty:
                ...
                # Adicionado QTDE_REGISTROS no df
                df = df[["ORGAO","CODIGO_SEFAZ", "CHAVE_FOLHA", "DESCRICAO", "DATA_FECHAMENTO", "DESCRICAO_TIPO", "TIPO_ARQUIVO", "QTDE_REGISTROS"]]

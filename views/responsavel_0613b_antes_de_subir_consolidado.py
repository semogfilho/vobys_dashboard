...
def render(conn, ano_selecionado, mes_chave, meses_disponiveis):
    ...
    #df_resultado['FOLHAS_FECHADAS'] = df_resultado['FOLHAS_F_TOTAL'] - df_resultado['FOLHAS_F_ABERTAS']
    ...
    for responsavel in ['JOSE GOMES', 'PEDRO MENDES', 'ARMANDO', 'DEIVIDY', 'SARAH', 'DEMAIS']:
        ...
        with st.expander(titulo_expander_consolidado, expanded=(responsavel == 'JOSE GOMES')):
            ...
            #df_filtrado = df_filtrado.drop(columns=['FOLHAS_F_TOTAL', 'FOLHAS_F_ABERTAS'])
            ...
            df_colorido = df_filtrado.style.apply(colorir_status, axis=1) \
                                     .set_properties(**{'text-align': 'center'}, subset=['STATUS', 'FOLHAS_FECHADAS'])
            st.dataframe(
                df_colorido, 
                use_container_width=True, # Ajuste automático ao container (autosize)
                hide_index=True,
                column_config={
                    "ORGAO": st.column_config.TextColumn("\u00D3RG\u00C3O / SCHEMA"),
                    "STATUS": st.column_config.TextColumn("STATUS ATUAL"),
                    "FOLHAS_FECHADAS": st.column_config.NumberColumn(
                        "QTD FECHADAS", 
                        format="%d",
                        width="small"
                    ),

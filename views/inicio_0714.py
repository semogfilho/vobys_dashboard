...
def render(conn, ano_selecionado, mes_chave, meses_disponiveis):
    ...
    with st.spinner("Compilando métricas..."):
        try:
            ...
            if not rows:
                # Cria um DataFrame vazio com a estrutura necessária para o código não quebrar
            ...
            # --- CÁLCULOS SEGUROS (MESMO SE O DF ESTIVER VAZIO) ---
            ...
            with c5:
                ...
                if total_erros > 0:
                    with st.popover("Ver Erros"):
                        ...
                        # 1. Criar uma cópia apenas para processamento visual, sem afetar o DataFrame original
                        df_exibicao = df_detalhe.copy()

                        # 2. Criar a coluna de link limpo (sem o /alterar)
                        df_exibicao['LINK_LIMPO'] = df_exibicao['LINK_EVENTO'].str.replace('/alterar', '', regex=False)

                        # 3. Exibição da tabela

                        # Exibição da tabela interna com a nova largura expandida
                        st.dataframe(
                            df_exibicao[['LINK_LIMPO', 'TIPO_REQ', 'NUM_RECIBO', 'DESCRICAO']], 
            ...
            # Garante o fechamento correto de todos os parênteses

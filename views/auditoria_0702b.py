                ...
                with st.spinner("Processando múltiplos payloads..."):
                    ...
                    if erro:
                        ...
                        if len(dif_folha) == 0 and len(dif_json) == 0 and len(cpfs_faltantes_na_folha) == 0 and len(cpfs_faltantes_no_json) == 0:
                            ...
                            with aba1:
                                if not dif_folha.empty: st.dataframe(dif_folha, use_container_width=True)
                                else: st.info("Nenhum registro divergente na folha.")

                            with aba2:
                                if not dif_json.empty: st.dataframe(dif_json, use_container_width=True)
                                else: st.info("Nenhum registro divergente no JSON SEFAZ.")

                            with aba3:
                                if not cpfs_faltantes_na_folha.empty: st.dataframe(cpfs_faltantes_na_folha, use_container_width=True)
                            ...
                            with aba4:
                                if not cpfs_faltantes_no_json.empty:
                                    ...
                                    if 'cod_limpo' in df_exibir.columns: df_exibir = df_exibir.drop(columns=['cod_limpo'])
                                    if 'KEY' in df_exibir.columns: df_exibir = df_exibir.drop(columns=['KEY'])

                                    # AJUSTE: Ordenar pelo Saldo Líquido de forma descendente

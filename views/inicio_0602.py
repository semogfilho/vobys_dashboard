...
def render(conn, ano_selecionado, mes_chave, meses_disponiveis):
    ...
    with st.spinner("Compilando métricas..."):
        try:
            ...
            total_geral = int(df_pivot["TOTAL"].sum())
            total_abertos = int(df_pivot[df_pivot['STATUS_INTEGRA'] == 'ABERTO']['TOTAL'].sum())
            
            # Alertas: Inconsistências e Erros
            total_erros = int(df_pivot[df_pivot['STATUS_INTEGRA'].str.contains('ERRO|INCONSISTENCIA', na=False)]['TOTAL'].sum())
            pct_erros = (total_erros / total_geral * 100) if total_geral > 0 else 0.0

            # CÁLCULO DE EFICIÊNCIA: TRANSMITIDO, FECHADO e ABERTO são válidos
            total_fechados = int(df_pivot[df_pivot['STATUS_INTEGRA'] == 'FECHADO']['TOTAL'].sum())
            total_transmitidos = int(df_pivot[df_pivot['STATUS_INTEGRA'] == 'TRANSMITIDO']['TOTAL'].sum())
            ...
            with c5:
                ...
                if total_erros > 0:
                    with st.popover("Ver Erros"):
                        # --- Botão na esquerda (col_pop_btn) para fugir dos comandos do Streamlit ---
                        col_pop_btn, col_pop_titulo = st.columns([1, 2])

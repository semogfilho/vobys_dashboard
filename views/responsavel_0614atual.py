...
def render(conn, ano_selecionado, mes_chave, meses_disponiveis):
    ...
    # --- RESUMO CONSOLIDADO ---
    st.markdown(f"""
        <div style="background-color: #1e3a8a; padding: 8px 15px; margin-bottom: 15px; border-radius: 6px; border-left: 5px solid #3b82f6;">
            <span style="color: #ffffff; font-weight: bold; font-size: 1.15rem;">📊 CONSOLIDADO GERAL ({mes_nome}/{ano_selecionado})</span>
        </div>
    """, unsafe_allow_html=True)

    tot_f_geral = df[['N_TOT', 'E_TOT']].sum().sum()
    ab_f_geral = df[['N_AB', 'E_AB']].sum().sum()
    indice_geral = ((tot_f_geral - ab_f_geral) / tot_f_geral * 100) if tot_f_geral > 0 else 100.0

    c1, c2, c3 = st.columns(3)
    c1.metric("Total Órgãos", len(df))
    c2.metric("Volumetria Total", tot_f_geral)
    c3.metric("Índice Performance", f"{indice_geral:.1f}%")
    ...
    # --- LISTAGEM RESPONSÁVEIS (RETOMADO FORMATO ORIGINAL) ---
    for responsavel in ['JOSE GOMES', 'PEDRO MENDES', 'ARMANDO', 'DEIVIDY', 'SARAH', 'DEMAIS']:
        ...
        with st.expander(titulo):
            ...
            # Retomando o uso do st.dataframe com estilo aplicado
            df_styled = df_resp[['ORGAO', 'STATUS', 'CHAVES']].style.apply(colorir_status, axis=1)

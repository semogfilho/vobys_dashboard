...
# Opção 3: Dados Bancários
         ...
         if not st.session_state.df_bancario.empty:
             ...
             # Processamento pendente (Tela de Envio/Visualização de JSON)
             if 'processamento_pendente' in st.session_state and st.session_state.processamento_pendente:
                 ...
                 for idx, registro in enumerate(registros):
                     ...
                     if registro.get("SOMENTE_VISUALIZAR", True):
                         ...
                         if not auth_ui.verificar_credenciais_sefaz():
                             st.warning("⚠️ Credenciais SEFAZ não validadas.")
                         else:
                             try:
                     ...
                     # BOTÃO GARANTIDO: Aparece em cada registro processado com chave única
                     if st.button("Voltar ao Painel", key=f"btn_voltar_painel_{cpf_reg}_{registro['COD_INSTITUCIONAL']}"):
                 ...
                 # Busca avulsa por CPF usando Session State (Evita DuplicateWidgetID)

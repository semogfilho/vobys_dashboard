...
# Opção 3: Controle SEFAZ
        ...
        if not st.session_state.df_bancario.empty:
            ...
            # Adicionamos uma coluna de link ou texto de suporte
            df_exibicao['AÇÃO'] = df_exibicao.apply(
                lambda x: "Ver Detalhe" if x['ENVIADO'] == '❌ ERRO' else "", axis=1
            )

            with st.form("form_envio_bancario"):
                # Ajuste: Adicionado on_select para capturar a linha clicada
                df_editado = st.data_editor(
                    df_exibicao,
                    column_config={
                        "ENVIAR": st.column_config.CheckboxColumn("Enviar?", default=False),
                        "ENVIADO": st.column_config.TextColumn("ENVIADO", help="✅ Sim | ❌ Erro | ⏳ Não"),
                    },
            ...
            st.subheader("🔍 Auditoria de Erros")
                ...
                # 2. Em vez de on_select, usamos um selectbox simples para escolher o CPF que deu erro
                # Isso funciona em TODAS as versões do Streamlit
            df_erros = df_editado[df_editado['ENVIADO'] == '❌ ERRO']
            ...
            if not df_erros.empty:
                ...
                if st.button("Carregar Log do Servidor"):
                    # Busca no banco apenas o log desse CPF

...
def render(conn, ano_selecionado, mes_chave, meses_disponiveis):
    ...
    with st.spinner("Realizando batimento bidirecional..."):
        try:
            ...
            # --- 2. PROCESSAMENTO REVERSO: FÍSICO -> BANCO (COM DETECÇÃO DE DUPLICIDADE) ---
            pastas_alvo = [
                ...
                if os.path.exists(pasta):
                    for arquivo in os.listdir(pasta):
                        if arquivo.endswith('.csv') and competencia_busca in arquivo:
                            ...
                            # Identifica categoria do arquivo físico
                            ...
                            # Controle de Duplicidade no Storage Físico
                            if arquivo in rastreio_arquivos_vistos:
                                timestamp_mod = os.path.getmtime(caminho_completo)
                                data_str = datetime.fromtimestamp(timestamp_mod).strftime('%d/%m/%Y %H:%M:%S')

                                arquivos_em_duplicidade.append({
                                    "CATEGORIA": nome_categoria,
                                    "ARQUIVO_DUPLICADO": arquivo,
                                    "DATA_MODIFICACAO": data_str,
                                    "CAMINHO_COMPLETO": caminho_completo,
                                    "STATUS": "Arquivo Físico Duplicado no Storage"
                                })
                            else:
                                rastreio_arquivos_vistos[arquivo] = caminho_completo

                            # Valida órfãos (não mapeados no banco)
                            if arquivo not in set_arquivos_banco:
                                ...
                                arquivos_orfaos_no_banco.append({
                                    ...
                                    "DATA_MODIFICACAO": data_arquivo_str,
                                    "CAMINHO_COMPLETO": caminho_completo,
                                    "STATUS": "Não Mapeado nos Metadados"
                                })

            # --- 2.1. DETECÇÃO DE DUPLICIDADE LÓGICA (METADADOS/BANCO) ---
            sql_duplicidade_banco = f"""
                SELECT
                    LPAD(codigo_sefaz, 3, '0') AS codigo_sefaz,
                    tipo_arquivo,
                    codigo_relatorio,
                    ind_tipo_requisicao,
                    tipo_folha_sefaz,
                ...
                HAVING count(0) > 1
            """
            cursor.execute(sql_duplicidade_banco)
            rows_dup_banco = cursor.fetchall()

            for cod_sef, t_arq, cod_rel, ind_req, t_folha, qtde in rows_dup_banco:
                tipo_limpo = str(ind_req).strip().upper()
                if tipo_limpo == 'V1':
                    cat_desc = "SIAPE-COLABORADORES (V1)"
                elif tipo_limpo == 'V2':
                    cat_desc = "SIAPE-CREDITOS (V2)"
                elif tipo_limpo == 'V4':
                    cat_desc = "SIAPE_FOLHA_PATRONAL (V4)"
                else:
                    cat_desc = "SIAPE-FOLHA_ORCAMENTARIO (V3)"

                sub_t_folha = str(t_folha).strip()[1:2] if t_folha and len(str(t_folha).strip()) >= 2 else "X"
                prefixo_arq = "FP_COL_" if tipo_limpo == 'V1' else ("FP_CRE_" if tipo_limpo == 'V2' else "FP_")
                nome_arq_gerado = f"{prefixo_arq}{cod_sef}_{sub_t_folha}_{ano_selecionado}{int(mes_chave):02d}_{t_arq}_{cod_rel}.csv"

                # Descobre o caminho físico padrão para exibir na listagem de duplicados, se existir
                if tipo_limpo == 'V1':
                    pasta_alvo_dup = '/home/vobys/ftp/data/SIAPE-COLABORADORES/'
                elif tipo_limpo == 'V2':
                    pasta_alvo_dup = '/home/vobys/ftp/data/SIAPE-CREDITOS/'
                else:
                    pasta_alvo_dup = '/home/vobys/ftp/data/SIAPE-FOLHAS/'

                caminho_absoluto_dup = os.path.join(pasta_alvo_dup, nome_arq_gerado)
                if os.path.exists(caminho_absoluto_dup):
                    ts_mod = os.path.getmtime(caminho_absoluto_dup)
                    data_mod_str = datetime.fromtimestamp(ts_mod).strftime('%d/%m/%Y %H:%M:%S')
                else:
                    data_mod_str = "N/A (Múltiplos registros BD)"

                arquivos_em_duplicidade.append({
                    "CATEGORIA": cat_desc,
                    "ARQUIVO_DUPLICADO": nome_arq_gerado,
                    "DATA_MODIFICACAO": data_mod_str,
                    "CAMINHO_COMPLETO": caminho_absoluto_dup,
                    "STATUS": f"Registro Duplicado na Base de Dados (Qtd: {qtde})"
                })
            ...
            total_duplicados = len(arquivos_em_duplicidade)
            ...
            with c5:
                st.metric(
                    label="Em Duplicidade",
                    value=f"{total_duplicados}",
                    delta=f"{total_duplicados} dup" if total_duplicados > 0 else "Nenhuma",
                    delta_color="inverse" if total_duplicados > 0 else "normal"
                )
            ...
            aba1, aba2, aba3, aba4 = st.tabs([
                "📊 Visão Geral do Batimento",
                "📁 Arquivos Pendentes no Storage",
                "⚠️ Arquivos Sem Vínculo de Integração",
            ...
            with aba1:
                ...
                if total_faltantes == 0 and total_orfaos == 0 and total_duplicados == 0:
                    st.success("🎉 **Auditoria Concluída:** Sincronia de 100% atingida entre o mapeamento lógico e os diretórios do Storage, sem sobreposições.")
                ...
                if arquivos_orfaos_no_banco:
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "DATA_MODIFICACAO": st.column_config.TextColumn("DATA DO ARQUIVO"),
            ...
            with aba4:
                st.markdown("### 🔄 Arquivos em Duplicidade / Causadores de Sobreposição")
                if arquivos_em_duplicidade:
                    df_duplicados = pd.DataFrame(arquivos_em_duplicidade)
                    df_duplicados = df_duplicados.sort_values(by='CAMINHO_COMPLETO', ascending=True)
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            "CATEGORIA": st.column_config.TextColumn("DIRETÓRIO DA PASTA"),
                            "ARQUIVO_DUPLICADO": st.column_config.TextColumn("NOME DO ARQUIVO"),
                            "DATA_MODIFICACAO": st.column_config.TextColumn("DATA DO ARQUIVO"),
                            "CAMINHO_COMPLETO": st.column_config.TextColumn("ENDEREÇO ABSOLUTO"),
                            "STATUS": st.column_config.TextColumn("STATUS DA AUDITORIA")
                        }
                    ...
                    st.success("✅ Nenhum arquivo em duplicidade detectado nos diretórios do Storage para esta competência.")

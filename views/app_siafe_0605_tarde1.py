...
def render(conn, ano_selecionado, mes_chave, meses_disponiveis):
    ...
    st.markdown("""
        <style>
            ...
            /* Garante que o bloco superior tenha o mesmo alinhamento das colunas da tabela */
            div[data-testid="stVerticalBlock"] > div:first-child { margin-top: 0px !important; }
            
            /* Estilo dos cards de totais */
            .metric-card {
    ...
    titulo_grafico = "Painel de Controle de Folhas"
    st.title(f"📊 {titulo_grafico}")
    
    subtitulo = f"Distribuição volumétrica: {meses_disponiveis[mes_chave]}/{ano_selecionado}"
    ...
    try:
        ...
        with col_grafico:
            # Mantive sua lógica de gráfico, apenas limpei o espaçamento
            st.write("### Composição do Status")
            # ... (seu código de gráfico continua aqui) ...

        with col_metricas:
            st.markdown("### 📋 Resumo Operacional")
            
            # Aplicando cores leves nos totalizadores
            col_metricas.markdown("""
            ...
            """.format(total_geral), unsafe_allow_html=True)
            
            st.markdown("<br>", unsafe_allow_html=True)
            
            # Lógica de alertas com cores mais suaves
            total_falhas = df[df['STATUS'].str.contains('ERRO|INCONSISTENCIA')]['QTD'].sum()
            if total_falhas > 0:

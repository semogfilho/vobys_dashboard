...
def render(conn, ano_selecionado, mes_chave, meses_disponiveis):
    ...
    with col2:
        ...
        c1, c2, c3 = st.columns(3)
        c1.metric("Volumetria Total", df[['N_TOT', 'E_TOT']].sum().sum())
        c2.metric("Folhas Abertas", df[['N_AB', 'E_AB']].sum().sum())
    ...
    st.info(f"📈 **Indicador de Performance:** O sistema já concluiu {concluido:.1f}% da volumetria física. Restam {ab_geral} folhas.")


    #titulo_limpo = "Produtividade por Responsável"

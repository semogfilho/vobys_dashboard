...
# Opção 2: Dados Bancários
        ...
        if not st.session_state.df_bancario.empty:
            ...
            # FORMULÁRIO APENAS PARA EDIÇÃO E ENVIO
            ...
            # LÓGICA DO FORM FORA DO BLOCO WITH
            if submit_button:
            ...
            if btn_buscar and cpf_busca:
                ...
                if not cpf_na_lista.empty:
                    ...
                    df_ex_local = cpf_na_lista.copy()
                    if 'Enviar?' in df_ex_local.columns: df_ex_local = df_ex_local.drop(columns=['Enviar?'])
                    st.dataframe(df_ex_local)
                    if st.button("Voltar para lista completa"): st.rerun()
                else:
                    ...
                    if df_encontrado is not None and not df_encontrado.empty:
                        ...
                        st.success("CPF localizado e adicionado!")
                        st.dataframe(df_encontrado)
                        if st.button("Voltar para lista completa"): st.rerun()
                    else:
                        st.error("CPF não encontrado.")
                        st.button("Tentar outro CPF")
            ...
            if 'processamento_pendente' in st.session_state and st.session_state.processamento_pendente:
                ...
                for registro in registros:
                    with status_container:
                        ...
                        try:
		            # --- BLOCO DE TESTE: SIMULAÇÃO SEM ENVIO REAL ---
                            sucesso = True  # Você pode alternar para False para testar o erro
                            json_str = "SIMULADO_JSON_PAYLOAD"
                            retorno = "Simulação de envio bem-sucedida (Sem acionar SEFAZ)."
                            # ------------------------------------------------

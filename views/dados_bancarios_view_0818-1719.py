...
def renderizar_dados_bancarios(conn, ano, mes, auth_ui, novos_dados_bancario):
    ...
    if not st.session_state.df_bancario.empty:
        ...
        # -------------------------------------------------------------
        # TELA 2: PAINEL PRINCIPAL / AUDITORIA (Com st.form para evitar refresh antecipado)
        # -------------------------------------------------------------
        else:
            df_exibicao = st.session_state.df_bancario.copy()
            df_exibicao['CPF'] = df_exibicao['CPF'].astype(str).str.replace(r'\D', '', regex=True)

            # Formatação segura da data para evitar tela em branco por dados nulos ou inválidos
            ...
            # Utilizando st.form para permitir marcar quantos itens quiser sem disparar reruns incômodos
            ...
            # Processamento dos botões do formulário
            if submit_button:

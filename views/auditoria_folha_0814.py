...
# --- OPÇÃO 4: DADOS BANCÁRIOS ---
    elif sub_opcao == "Dados Bancários":
        mes_exibicao = "13º" if int(mes) == 13 else f"{int(mes):02d}"
        st.subheader(f"🏦 Dados Bancários (Controle SEFAZ) ({mes_exibicao}/{ano})")

        # [AJUSTE]: A verificação de credenciais SEFAZ foi removida daqui
        # para permitir visualização direta dos novos usuários.
        ...
        if not st.session_state.df_bancario.empty:
            ...
            if submit_button:
                ...
                if selecionados.empty:
                    ...
                    # [AJUSTE PRINCIPAL]: Exige as credenciais SEFAZ apenas no momento do envio efetivo
                    enviar_efetivo = selecionados[selecionados["SOMENTE_VISUALIZAR"] == False]
                    
                    if not enviar_efetivo.empty:
                        if not auth_ui.verificar_credenciais_sefaz():
                            st.stop()  # Exibe o form de login SEFAZ e interrompe a execução até autenticar
            ...
            if btn_buscar and cpf_busca:
                ...
                if st.button("⬅️ Voltar para lista completa"): 
                    st.rerun()

            # PROCESSAMENTO DOS SELECIONADOS
            if 'processamento_pendente' in st.session_state and st.session_state.processamento_pendente:
                ...
                for registro in registros:
                    ...
                    if registro.get("SOMENTE_VISUALIZAR", True):
                        ...
                        # [AJUSTE SECUNDÁRIO]: Garantia adicional de autenticação antes de disparar o webservice
                        if not auth_ui.verificar_credenciais_sefaz():
                            st.stop()

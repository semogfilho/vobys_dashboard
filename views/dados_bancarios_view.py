    ...
    try:
        ...
        if 'df_bancario' not in st.session_state or st.session_state.get('last_params') != (ano, mes):
            with st.spinner("Buscando dados no banco..."):
                ...
                # =========================================================
                # FILTRO PARA EXIBIR APENAS A SEDUC
                # =========================================================
                #if 'ORGAO' in df_temp.columns:
                #    df_temp = df_temp[df_temp['ORGAO'].astype(str).str.upper() == 'SEDUC'].reset_index(drop=True)
                # =========================================================

                # =========================================================
                # LIMITAR APENAS AOS 200 PRIMEIROS REGISTROS
                # =========================================================
                #df_temp = df_temp.head(200).reset_index(drop=True)
                # =========================================================
        ...
        if not st.session_state.df_bancario.empty:
            ...
            # -------------------------------------------------------------
                ...
                if "resumo_checar_sefaz" in st.session_state:
                    resumo_dados = st.session_state.resumo_checar_sefaz.get("resultados", [])
                    if resumo_dados:
                        # Conta as ocorrências por status
                        contagem_status = {}
                        for _, _, _, _, status in resumo_dados:
                            contagem_status[status] = contagem_status.get(status, 0) + 1

                        with st.popover("📊 Ver Resumo da Checagem SEFAZ", use_container_width=True):
                            st.markdown("### Resumo da Checagem Realizada")
                            st.write(f"Total de registros processados neste lote: **{len(resumo_dados)}**")
                            st.divider()
                            for st_nome, qtd in contagem_status.items():
                                st.markdown(f"- **{st_nome}**: `{qtd}` registros")
                            
                            if st.button("🔄 Fechar / Limpar Resumo", key="btn_fechar_resumo_popover"):
                    ...
                    if indices_pendentes:
                        if chk_usar_passo_a_passo:
                            ...
                            for i, idx in enumerate(indices_pendentes):
                                ...
                                st.session_state.df_bancario.loc[idx, 'SEFAZ'] = res_visual
                                resultados_lote.append((idx, cpf_cru, matricula_atual, nome_atual, res_visual))
                            ...
                            # Armazena o resumo no session_state para exibir no popover

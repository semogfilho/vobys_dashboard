...
def formatar_cpf(val):
  ...
  if len(val_limpo) == 11:
    return f"{val_limpo[:3]}.{val_limpo[3:6]}.{val_limpo[6:9]}-{val_limpo[9:]}"
  return val
    ...
    try:
      ...
      if st.button("Processar e Gerar Arquivo Final"):
        ...
        try:
          ...
          df_vinculo.columns = [
              str(c).lower().strip() for c in df_vinculo.columns
          ]
          ...
          df_agencia.columns = [
              str(c).lower().strip() for c in df_agencia.columns
          ]
          ...
          total_encontrados = (
              (df_saida["ID_PESSOA"] != "") & (df_saida["ID_PESSOA"].notnull())
          ).sum()

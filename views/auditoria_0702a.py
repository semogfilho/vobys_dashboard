...
def render(conn, ano, mes):
    ...
    if sub_opcao == "Consistência Folha":
        ...
        query_ids = f"""
            ...
            WHERE sei.ano = {ano} AND sei.mes = {mes} AND sei.ind_tipo_requisicao = 'V1' --and not chave_folha like '%-450'
        """
        df_opcoes = pd.read_sql(query_ids, conn)
        df_opcoes.columns = [c.lower() for c in df_opcoes.columns]

        # Agora o display fica mais informativo
        df_opcoes['display'] = (
...
# 1. Filtro de pesquisa textual
        ...
        # 2. Filtra o DF de opções baseado no texto digitado
        if pesquisa:
        ...
        # Multiselect ocupa toda a largura da coluna principal
        selecionados = st.multiselect(
            "Selecione os IDs para Batimento:",
            #options=df_opcoes['id_siafe_evento_integracao'].tolist(),
        ...
        if st.button("Executar Batimento Consolidado"):
            if not selecionados:
                ...
                with st.spinner("Processando múltiplos payloads..."):
                    from auditoria import batimento_json
                    
                    # Chamada atualizada: note que removemos 'chave_selecionada'
                    dif_folha, dif_json, pct_folha, pct_json, erro, df_folha_total, df_json_total = batimento_json.processar_batimento_consolidado(conn, selecionados)

                    if erro:
                        st.error(f"Erro ao processar: {erro}")
                    else:
                        # Criação de colunas para métricas (Dashboard de Sênior)
                       # st.write("Colunas no df_json_total:", df_json_total.columns.tolist())
                       # st.write("Colunas no df_folha_total:", df_folha_total.columns.tolist())


                        col1, col2 = st.columns(2)
    
                        col1.metric("Divergências na Folha", f"{len(dif_folha)}", f"{pct_folha:.2f}% de inconsistência")
                        col2.metric("Divergências no JSON", f"{len(dif_json)}", f"{pct_json:.2f}% de inconsistência")
	                ...
	                # ADICIONE ESTE BLOCO ABAIXO PARA EXIBIR A LISTAGEM
			...
			# No seu auditoria_folha.py, antes de exibir:
                        dif_folha = dif_folha.reset_index(drop=True)
                        dif_json = dif_json.reset_index(drop=True)
                        cpfs_faltantes_na_folha = cpfs_faltantes_na_folha.reset_index(drop=True)
                        ...
                        with aba3:
                            if not cpfs_faltantes_na_folha.empty:
                                st.warning(f"Existem {len(cpfs_faltantes_na_folha)} registros no JSON que não foram encontrados na Folha.")
                                st.dataframe(cpfs_faltantes_na_folha, use_container_width=True)
                            else:
                                st.success("Tudo sincronizado! Não há CPFs no JSON que faltem na Folha.")

                        with aba4:
                            if not cpfs_faltantes_no_json.empty:
                                resultados_finais = []
                                # Agrupamos por ORGAO (schema) para garantir que buscamos na tabela certa
                                ...
                                if 'cod_limpo' in df_exibir.columns:
                                    df_exibir = df_exibir.drop(columns=['cod_limpo'])
                                if 'KEY' in df_exibir.columns:
                                    df_exibir = df_exibir.drop(columns=['KEY'])
                                ...
                                st.dataframe(
                                    ...
                                    column_config={
                                        "SALDO_LIQUIDO": st.column_config.NumberColumn("Saldo Líquido", format="R$ %.2f")
                                    }
                                )
                            else:

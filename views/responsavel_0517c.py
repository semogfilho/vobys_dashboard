...
def render(conn, ano_selecionado, mes_chave, meses_disponiveis):
    ...
    with st.spinner("Sincronizando metadados de auditoria e conformidade..."):
        for resp, schema in todos_schemas_mapeados:
            ...
            # Contabilidade física das folhas internas
            qtd_folhas_totais = 0
    ...
    for responsavel in ['JOSE GOMES', 'PEDRO MENDES', 'DEMAIS']:
        ...
        # Contabilidade estrutural precisa para a barra mestre
        ...
        # Coleta volumétrica física real vindas do loop do banco
        ...
        # Montagem do cabeçalho idêntico à imagem de referência
        texto_cabecalho = (
            f".. RESPONS\u00C1VEL: {responsavel} | "
            f"\u00D3rg\u00E3os: {total_orgaos} (Abertos: {num_abertas} / Fechados: {num_fechadas}) | "
            f"[VOLUMETRIA FOLHAS] Total: {tot_folhas_fisicas} | Abertas: {abertas_folhas_fisicas} | "
            f"\u00CDndice: {taxa_eficiencia:.1f}%"
        )

        # O componente mestre/detalhe que engloba tudo
        with st.expander(texto_cabecalho, expanded=(responsavel == 'JOSE GOMES')):
            
            # --- AJUSTE SOLICITADO: Rótulos corrigidos para Órgãos ---
            m1, m2, m3, m4 = st.columns(4)
            with m1:
                st.metric(label="Total \u00D3rg\u00E3os", value=f"{total_orgaos}")
            with m2:
                st.metric(label="\u00D3rg\u00E3os Abertos", value=f"{num_abertas}")
            with m3:
                st.metric(label="\u00D3rg\u00E3os Fechados", value=f"{num_fechadas}")
            with m4:
                st.metric(label="\u00CDndice de Fechamento", value=f"{taxa_eficiencia:.1f}%")

            # Preparação da grid visual de dados (Mapeamento original)

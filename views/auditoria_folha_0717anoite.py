...
# Opção 2: Dados Bancários
        ...
        if not st.session_state.df_bancario.empty:
            ...
            if "SOMENTE_VISUALIZAR" not in st.session_state.df_bancario.columns:
                st.session_state.df_bancario["SOMENTE_VISUALIZAR"] = True
            ...
            with st.form("form_envio_bancario"):
                df_editado = st.data_editor(
                    ...
                    column_config={
                        ...
                        "SOMENTE_VISUALIZAR": st.column_config.CheckboxColumn("Visualizar?", default=True)
                    },
            ...
            if submit_button:
                ...
                st.session_state.df_bancario["SOMENTE_VISUALIZAR"] = df_editado["SOMENTE_VISUALIZAR"]
            ...
            # ÁREA DE BUSCA (CPF AVULSO)
            st.divider()
            # CSS para forçar o alinhamento vertical das colunas
            st.markdown("""
                <style>
                /* Alinha o conteúdo das colunas verticalmente na base */
                [data-testid="column"] {
                    display: flex;
                    align-items: flex-end;
                }
                
                /* Remove a margem superior do container que contém o botão */
                div.stButton {
            ...
            with col2:
                # O botão agora deve se alinhar com a base do input
                btn_buscar = st.button("Buscar CPF na Competência", use_container_width=True)
            ...
            if btn_buscar and cpf_busca:
                ...
                if st.session_state.df_bancario[mask_cpf].any().any():
                    ...
                    st.data_editor(st.session_state.df_bancario[mask_cpf], hide_index=True, use_container_width=True)
                else:
                    ...
                    if df_encontrado is not None and not df_encontrado.empty:
                        ...
                        df_encontrado['SOMENTE_VISUALIZAR'] = True
                        ...
                        st.success(f"CPF {cpf_limpo} localizado e adicionado!")
                    else:
                        st.error("CPF não encontrado na folha desta competência.")
                if st.button("⬅️ Voltar para lista completa"): st.rerun()
            ...
            if 'processamento_pendente' in st.session_state and st.session_state.processamento_pendente:
                ...
                for registro in registros:
                    ...
                    # 2. LÓGICA DE VISUALIZAÇÃO OU ENVIO
                    if registro.get("SOMENTE_VISUALIZAR", True):
                        ...
                        try:
                            ...
                            if sucesso:
                                ...
                                st.error(f"Erro no envio de {registro.get('NOME_ATUAL')}: {retorno}")
                        except Exception as e:
                            st.error(f"Erro sistêmico: {e}")
                            
                    # 3. A LIMPEZA OCORRE AQUI, APÓS O IF/ELSE, USANDO O MASK DEFINIDO ANTES

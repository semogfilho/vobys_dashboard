...
def modal_autenticacao_sefaz(servico_destino):
    # CSS para dar aquele toque de "janela" da SEFAZ
    st.markdown("""
        <style>
        [data-testid="stDialog"] {
            border-top: 5px solid #005aab; /* Azul característico */
            border-radius: 10px;
        }
        .stButton>button {
    ...
    st.write(f"Informe as credenciais para: **{servico_destino}**")
    
    # Layout igual ao que você enviou
    with st.form("form_sefaz_estilo"):
        cpf = st.text_input("Usuário (CPF)")
        senha = st.text_input("Senha", type="password")
        exercicio = st.selectbox("Exercício", [2026, 2025, 2024])
        
        # Botão estilizado para parecer com o "Ok" da imagem
        submitted = st.form_submit_button("✅ Ok")
        
        if submitted:
            # Sua lógica de validação aqui
            if auth_ui.validar_sefaz(cpf, senha):
                st.session_state["sefaz_cpf"] = cpf
                st.session_state["usuario_envio_completo"] = f"{st.session_state['login_atual']}_{cpf}"
                st.rerun()
            else:
...
def verificar_credenciais_sefaz():
    ...
    if 'sefaz_auth' not in st.session_state:
        st.session_state.sefaz_auth = False

    # 2. SE JÁ ESTIVER LOGADO: Mostra o status e o botão de desconectar na sidebar
    if st.session_state.get('sefaz_auth'):
        ...
        if st.sidebar.button("Desconectar SEFAZ"):
            ...
            st.rerun() # Recarrega para limpar a tela e voltar ao estado de "não logado"
        return True # Retorna True pois o usuário já está autenticado

    # 3. SE NÃO ESTIVER LOGADO: Exibe o formulário
    with st.form("form_sefaz_login"):
        st.write("### Acesso Requerido: SEFAZ")
        user_cpf = st.text_input("CPF SEFAZ")
        user_pass = st.text_input("Senha SEFAZ", type="password")
    ...
    return False # Retorna False pois o usuário precisou (ou tentou) logar agora
